...
class MechaAutoshipImageColorFilter(Node):
    ...
    def timer_callback(self):
        if len(self.origin_img) != 0:
            ...
            get_shape = self.get_image_shape(filter_img)
            print("##########################################")
            print(get_shape)
            self.publishers_.publish(get_shape)
            self.get_logger().info("Publish shape")
    ...
    def get_image_shape(self, img):
        ...
        target_tri = cv2.imread('~/tri.jpg')#타겟 이미지 : plus.jpg, tri.jpg, circle.jpg
        targetGray_T = cv2.cvtColor(target_tri, cv2.COLOR_BGR2GRAY)
        ret1, targetTh_T = cv2.threshold(targetGray_T, 127, 255, cv2.THRESH_BINARY_INV)
        target_plus = cv2.imread('~/tri.jpg')#타겟 이미지 : plus.jpg, tri.jpg, circle.jpg
        targetGray_P = cv2.cvtColor(target_plus, cv2.COLOR_BGR2GRAY)
        ret2, targetTh_P = cv2.threshold(targetGray_P, 127, 255, cv2.THRESH_BINARY_INV)
        target_circle = cv2.imread('~/tri.jpg')#타겟 이미지 : plus.jpg, tri.jpg, circle.jpg
        targetGray_C = cv2.cvtColor(target_circle, cv2.COLOR_BGR2GRAY)
        ret3, targetTh_C = cv2.threshold(targetGray_C, 127, 255, cv2.THRESH_BINARY_INV)
        _ , contours, _ = cv2.findContours(img, cv2.RETR_EXTERNAL, cv2.CHAIN_APPROX_SIMPLE)
        _ , contoursT, _ = cv2.findContours(targetTh_T, cv2.RETR_EXTERNAL, cv2.CHAIN_APPROX_SIMPLE)
        _ , contoursP, _ = cv2.findContours(targetTh_P, cv2.RETR_EXTERNAL, cv2.CHAIN_APPROX_SIMPLE)
        _ , contoursC, _ = cv2.findContours(targetTh_C, cv2.RETR_EXTERNAL, cv2.CHAIN_APPROX_SIMPLE)

        matchs  = []
        # 대상 도형과 여러 도형 중 하나와 매칭 실행 ---①
        match_with_triangle = cv2.matchShapes(contoursT, contours, cv2.CONTOURS_MATCH_I2, 0.0)
        match_with_circle = cv2.matchShapes(contoursC, contours, cv2.CONTOURS_MATCH_I2, 0.0)
        match_with_plus = cv2.matchShapes(contoursP, contours, cv2.CONTOURS_MATCH_I2, 0.0)
        # 해당 도형의 매칭 점수와 컨투어를 쌍으로 저장 ---②
        matchs.append((match_with_triangle, contours, "triangle"))
        matchs.append((match_with_circle, contours, "circle"))
        matchs.append((match_with_plus, contours, "plus"))
